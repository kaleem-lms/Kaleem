...
active_spec: "none — **the session UI redesign was visually verified on staging 2026-09-08**, closing the D9 gap it shipped with: desktop three-zone grid, measured reflow (44px row → 96px stacked at 390px), RTL mirroring, the Lobby gesture gate with no getUserMedia before a tap, and no false diagnostic row from room navigation. Two findings logged, not fixed (Join/Cancel width mismatch on mobile; slots+quota 404-instead-of-403). none — the shared-signaling fix (ADR-0037) is SHIPPED and **verified live on staging 2026-09-07**. Signaling is now one shared service on `ws-staging.kaleem.academy`, started with the other shared services and never stopped by a colour flip. Verified by the check that matters: a room created before a deploy (id 6, session 5) still agreed with what the new colour served after a green→blue flip, and `ws-staging` returned 200 throughout. **Mutation-checked** — after the flip `ws-green-staging` returns Traefik's bare 404 with its router gone, so a room pinned the old way would now be dead. Migration 0009 ended every per-colour-pinned room (active count 0 afterwards). Two things stand: a deploy still drops calls in progress (the drain is unbuilt; deploy between lessons), and an operator who edits WS_DOMAIN without first ending ACTIVE rooms reopens the same failure for that one action — runbook + ISSUES.md, not code. Next phase not yet chosen."
...
last_green_ci: "meta c1a67c1 → master, 2026-09-07 (all 8 checks green, deploy-staging SUCCESS), re-run once more to flip the colour for the ADR-0037 live check. **Staging is on blue.** Signaling is now the shared `kaleem-signaling-1` on `ws-staging.kaleem.academy`, NOT per-colour — `ws-blue-staging`/`ws-green-staging` no longer route and their containers are gone."
---
