...
active_branch: "**`fix/dashboard-pointer-media-fallback` (meta #229) — OPEN, all ten checks green, NOT merged.** Merging it deploys. It bumps dashboard to 9d21e01 (#71): a combined `getUserMedia({video,audio})` is all-or-nothing, so a missing camera cost a working microphone — staging session 7 had user 18 failing `gum-not-found` on three separate days while user 17 in the same session never failed once. `useLocalMedia` now retries per kind. ⚠ The `backend` pointer is ALSO behind (c1c2819..2401dc7, its PR #63, already merged to backend/main) and was deliberately left out of that deploy — it still needs its own bump. meta #227 (an ISSUES note: one failed tap writes TWO diagnostics rows) is merged."
last_green_ci: "meta#229 (PR, unmerged) 2026-09-13: all ten checks green, including the FIRST EVER execution of `e2e/call-partial-media.spec.ts` — verified by reading the job log for the spec name (`✓ 21 … a missing camera no longer costs a working microphone`, 78 passed), not by trusting a green job, because a silently-skipped spec looks identical to a passing one. `dashboard-lint` also cleared the raised coverage floors (96.73/93.28/88.84) in CI, not just locally. `deploy-staging` correctly showed `skipping` — it runs on push to master, so the merge is the deploy."
---
