...
import { Book, Calendar, ChevronDown, FileText, Film, Plus, Search, User, Users, X } from 'lucide-react';
...
export default function StudentManagement() {
	...
	const studentReports = selectedStudent
		? mockSessionReports.filter((report) => report.student_id === selectedStudent.id)
		: [];
	...
	const removeResourceAssignment = (resourceId: number) => {
		...
		setStudentResources(
			studentResources.filter((sr) => !(sr.student_id === selectedStudent.id && sr.resource_id === resourceId)),
		);
	};
		...
		<div className="container mx-auto py-6">
			<div className="mb-6 flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Manage resources and view session reports for your students.</p>
				</div>
						...
						<ScrollArea className="h-[500px]">
							<div className="p-1">
								{filteredStudents.map((student) => (
									...
									>
										<Avatar className="h-9 w-9">
											...
											<AvatarFallback>{student.user.name.substring(0, 2).toUpperCase()}</AvatarFallback>
										</Avatar>
										<div className="flex-1 overflow-hidden">
											<p className="truncate font-medium leading-none">{student.user.name}</p>
											<p
												className={`truncate text-xs ${selectedStudent?.id === student.id ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}
											>
												{student.user.email}
											</p>
										</div>
								...
								{filteredStudents.length === 0 && (
									<div className="px-4 py-8 text-center text-muted-foreground">No students found</div>
								)}
							</div>
						</ScrollArea>
					</CardContent>
					...
					{selectedStudent ? (
							...
							<div className="mb-4 flex items-center justify-between">
								<div className="flex items-center gap-4">
									<Avatar className="h-12 w-12">
										<AvatarImage src={selectedStudent.user.profile.picture} alt={selectedStudent.user.name} />
										<AvatarFallback>{selectedStudent.user.name.substring(0, 2).toUpperCase()}</AvatarFallback>
									</Avatar>
									<div>
										<h2 className="font-bold text-2xl">{selectedStudent.user.name}</h2>
								...
								<Card>
									<CardHeader className="flex flex-row items-center justify-between">
										<div>
											...
											<CardDescription>Learning materials assigned to {selectedStudent.user.name}</CardDescription>
										</div>
										<Dialog open={isAssigningResource} onOpenChange={setIsAssigningResource}>
											...
											<DialogContent>
												<DialogHeader>
													...
													<DialogDescription>Select a resource to assign to {selectedStudent.user.name}</DialogDescription>
												</DialogHeader>
												<div className="py-4">
													<ScrollArea className="h-[300px]">
														{resources.map((resource) => {
															const isAssigned = studentResources.some(
																(sr) => sr.student_id === selectedStudent.id && sr.resource_id === resource.id,
															);

															return (
																<div key={resource.id} className="flex items-center justify-between rounded-md p-3 hover:bg-muted">
																	<div className="flex items-center gap-3">
																		<div className="rounded-md bg-primary/10 p-2">{getResourceIcon(resource.resource_type)}</div>
																		<div>
																	...
																	{isAssigned ? (
																		...
																		<Button size="sm" onClick={() => assignResourceToStudent(resource.id)}>
																			Assign
																		</Button>
																	)}
																</div>
												...
												<DialogFooter>
													<Button variant="outline" onClick={() => setIsAssigningResource(false)}>
														Cancel
													</Button>
												</DialogFooter>
											</DialogContent>
									...
									<CardContent>
										{assignedResources.length > 0 ? (
											<div className="grid gap-4">
												{assignedResources.map((resource) => (
													<div key={resource.id} className="flex items-center justify-between rounded-lg border p-4">
														<div className="flex items-center gap-4">
															<div className="rounded-md bg-primary/10 p-3">{getResourceIcon(resource.resource_type)}</div>
															<div>
														...
														<div className="flex items-center gap-2">
															...
															<Button variant="ghost" size="sm" onClick={() => removeResourceAssignment(resource.id)}>
																<X className="h-4 w-4" />
															</Button>
														</div>
													</div>
											...
											<div className="py-12 text-center">
												...
												<h3 className="mt-4 font-medium text-lg">No resources assigned</h3>
												<p className="mt-2 text-muted-foreground">
													Assign learning resources to help {selectedStudent.user.name} with their studies.
												</p>
												<Button className="mt-4" onClick={() => setIsAssigningResource(true)}>
							...
							<TabsContent value="reports">
								<Card>
									<CardHeader>
										...
										<CardDescription>Recent learning sessions with {selectedStudent.user.name}</CardDescription>
									</CardHeader>
									<CardContent>
										{studentReports.length > 0 ? (
												...
												<TableBody>
													{studentReports.map((report) => (
														<TableRow key={report.id}>
															...
															<TableCell className="max-w-[200px] truncate">{report.topics_covered}</TableCell>
															<TableCell>
																<Badge
																	variant={
											...
											<div className="py-12 text-center">
												...
												<h3 className="mt-4 font-medium text-lg">No session reports</h3>
												<p className="mt-2 text-muted-foreground">
													There are no recorded sessions with {selectedStudent.user.name} yet.
												</p>
								...
								{studentReports.length > 0 && (
									<Card className="mt-6">
										<CardHeader>
											...
											<CardDescription>{formatDate(studentReports[0].date)}</CardDescription>
										</CardHeader>
										<CardContent>
											<div className="grid gap-4">
												<div>
													<h4 className="mb-1 font-medium text-sm">Topics Covered</h4>
													<p>{studentReports[0].topics_covered}</p>
												</div>
												<Separator />
												<div>
													<h4 className="mb-1 font-medium text-sm">Progress</h4>
													<p>{studentReports[0].progress}</p>
												</div>
												<Separator />
												<div>
													<h4 className="mb-1 font-medium text-sm">Homework</h4>
						...
						<Card className="flex h-full items-center justify-center">
							<CardContent className="py-12 text-center">
								...
								<h3 className="mt-4 font-medium text-lg">No student selected</h3>
								<p className="mt-2 text-muted-foreground">
									Select a student from the list to view and manage their resources and reports.
								</p>
							</CardContent>
						</Card>
					)}
